 ...
 *
 * File name: uvc_jni_registration.cpp
 *
...
#ifndef IOCTL_USBFS_RESET
/* linux_usbfs.h: reset is available to apps with an open usbfs fd (no root). */
#define IOCTL_USBFS_RESET _IO('U', 20)
#endif

static jint nativeIoctlUsbReset(JNIEnv *env, jclass clazz, jint fd) {
	ENTER();
	if (fd < 0) {
		RETURN(-EINVAL, jint);
	}
	...
	if (r < 0) {
		LOGE("nativeIoctlUsbReset: fd=%d errno=%d", (int)fd, errno);
		RETURN(-errno, jint);
	}
	RETURN(0, jint);
}
...
static jobject nativeGetDescriptions(JNIEnv *env, jobject thiz,
	ID_TYPE id_camera) {
...
static jint nativeSetPreviewTransform(JNIEnv *env, jobject thiz,
	...
	jfloat scale, jfloat pan_x, jfloat pan_y, jfloat fit_x, jfloat fit_y) {
	UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
	if (LIKELY(camera))
		return camera->setPreviewTransform(rotation, flip_h == JNI_TRUE, flip_v == JNI_TRUE,
			scale, pan_x, pan_y, fit_x, fit_y);
	return JNI_ERR;
}
...
//======================================================================
// Java method corresponding to this function should not be a static method
...
//======================================================================
// Java method corresponding to this function should not be a static method
static jint nativeUpdateAutoWhiteBalanceLimit(JNIEnv *env, jobject thiz,
	ID_TYPE id_camera) {
	jint result = JNI_ERR;
	ENTER();
	UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
	if (LIKELY(camera)) {
		int min, max, def;
		result = camera->updateAutoWhiteBalanceLimit(min, max, def);
		if (!result) {
			// Java側へ書き込む
			setField_int(env, thiz, "mAutoWhiteBalanceMin", min);
			setField_int(env, thiz, "mAutoWhiteBalanceMax", max);
			setField_int(env, thiz, "mAutoWhiteBalanceDef", def);
		}
	}
	RETURN(result, jint);
}

static jint nativeSetAutoWhiteBalance(JNIEnv *env, jobject thiz,
	ID_TYPE id_camera, jboolean autofocus) {
	...
	UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
	if (LIKELY(camera)) {
		result = camera->setAutoWhiteBalance(autofocus);
	}
	RETURN(result, jint);
}

static jint nativeGetAutoWhiteBalance(JNIEnv *env, jobject thiz,
	ID_TYPE id_camera) {
	...
	UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
	if (LIKELY(camera)) {
		result = camera->getAutoWhiteBalance();
	}
	RETURN(result, jint);
}
...
//======================================================================
// Java method corresponding to this function should not be a static method
static jint nativeUpdateAutoWhiteBalanceCompoLimit(JNIEnv *env, jobject thiz,
	ID_TYPE id_camera) {
	jint result = JNI_ERR;
	ENTER();
	UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
	if (LIKELY(camera)) {
		int min, max, def;
		result = camera->updateAutoWhiteBalanceCompoLimit(min, max, def);
		if (!result) {
			// Java側へ書き込む
			setField_int(env, thiz, "mAutoWhiteBalanceCompoMin", min);
			setField_int(env, thiz, "mAutoWhiteBalanceCompoMax", max);
			setField_int(env, thiz, "mAutoWhiteBalanceCompoDef", def);
		}
	}
	RETURN(result, jint);
}

static jint nativeSetAutoWhiteBalanceCompo(JNIEnv *env, jobject thiz,
	...
	UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
	if (LIKELY(camera)) {
		result = camera->setAutoWhiteBalanceCompo(autofocus_compo);
	}
	RETURN(result, jint);
}

static jint nativeGetAutoWhiteBalanceCompo(JNIEnv *env, jobject thiz,
	ID_TYPE id_camera) {
	...
	UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
	if (LIKELY(camera)) {
		result = camera->getAutoWhiteBalanceCompo();
	}
	RETURN(result, jint);
}
...
//======================================================================
// Java method corresponding to this function should not be a static method
static jint nativeUpdateWhiteBalanceLimit(JNIEnv *env, jobject thiz,
	ID_TYPE id_camera) {
	jint result = JNI_ERR;
	ENTER();
	UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
	if (LIKELY(camera)) {
		int min, max, def;
		result = camera->updateWhiteBalanceLimit(min, max, def);
		if (!result) {
			// Java側へ書き込む
			setField_int(env, thiz, "mWhiteBalanceMin", min);
			setField_int(env, thiz, "mWhiteBalanceMax", max);
			setField_int(env, thiz, "mWhiteBalanceDef", def);
		}
	}
	RETURN(result, jint);
}
...
static jint nativeSetWhiteBalance(JNIEnv *env, jobject thiz,
	...
	UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
	if (LIKELY(camera)) {
		result = camera->setWhiteBalance(whiteBalance);
	}
	RETURN(result, jint);
}
...
//======================================================================
// Java method corresponding to this function should not be a static method
static jint nativeUpdateWhiteBalanceCompoLimit(JNIEnv *env, jobject thiz,
	ID_TYPE id_camera) {
	jint result = JNI_ERR;
	ENTER();
	UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
	if (LIKELY(camera)) {
		int min, max, def;
		result = camera->updateWhiteBalanceCompoLimit(min, max, def);
		if (!result) {
			// Java側へ書き込む
			setField_int(env, thiz, "mWhiteBalanceCompoMin", min);
			setField_int(env, thiz, "mWhiteBalanceCompoMax", max);
			setField_int(env, thiz, "mWhiteBalanceCompoDef", def);
		}
	}
	RETURN(result, jint);
}
...
static jint nativeSetWhiteBalanceCompo(JNIEnv *env, jobject thiz,
	...
	UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
	if (LIKELY(camera)) {
		result = camera->setWhiteBalanceCompo(whiteBalance_compo);
	}
	RETURN(result, jint);
}
...
static jint nativeGetWhiteBalanceCompo(JNIEnv *env, jobject thiz,
	...
	UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
	if (LIKELY(camera)) {
		result = camera->getWhiteBalanceCompo();
	}
	RETURN(result, jint);
}
...
static JNINativeMethod methods[] = {
	{ "nativeIoctlUsbReset",			"(I)I", (void *) nativeIoctlUsbReset },
	...
	{ "nativeSetStatusCallback",		"(JLorg/centennialoss/consolation/uvc/IStatusCallback;)I", (void *) nativeSetStatusCallback },
	{ "nativeSetButtonCallback",		"(JLorg/centennialoss/consolation/uvc/IButtonCallback;)I", (void *) nativeSetButtonCallback },
	...
	{ "nativeSetPreviewTransform",		"(JIZZFFFFF)I", (void *) nativeSetPreviewTransform },
...
//	{ "nativeGetFocusSimple",			"(J)I", (void *) nativeGetFocusSimple },
	...
	{ "nativeUpdateAutoWhiteBalanceLimit","(J)I", (void *) nativeUpdateAutoWhiteBalanceLimit },
	{ "nativeSetAutoWhiteBalance",		"(JZ)I", (void *) nativeSetAutoWhiteBalance },
	{ "nativeGetAutoWhiteBalance",		"(J)I", (void *) nativeGetAutoWhiteBalance },

	{ "nativeUpdateAutoWhiteBalanceCompoLimit","(J)I", (void *) nativeUpdateAutoWhiteBalanceCompoLimit },
	{ "nativeSetAutoWhiteBalanceCompo",		"(JZ)I", (void *) nativeSetAutoWhiteBalanceCompo },
	{ "nativeGetAutoWhiteBalanceCompo",		"(J)I", (void *) nativeGetAutoWhiteBalanceCompo },
	
	{ "nativeUpdateWhiteBalanceLimit",	"(J)I", (void *) nativeUpdateWhiteBalanceLimit },
	{ "nativeSetWhiteBalance",			"(JI)I", (void *) nativeSetWhiteBalance },
	{ "nativeGetWhiteBalance",			"(J)I", (void *) nativeGetWhiteBalance },

	{ "nativeUpdateWhiteBalanceCompoLimit","(J)I", (void *) nativeUpdateWhiteBalanceCompoLimit },
...
int register_uvccamera(JNIEnv *env) {
	...
	if (registerNativeMethods(env,
		"org/centennialoss/consolation/uvc/UVCCamera",
