...
public final class USBMonitor {
	...
	private static final String ACTION_USB_PERMISSION_BASE = "org.centennialoss.consolation.USB_PERMISSION.";
	private final String ACTION_USB_PERMISSION = ACTION_USB_PERMISSION_BASE + hashCode();

	public static final String ACTION_USB_DEVICE_ATTACHED = "android.hardware.usb.action.USB_DEVICE_ATTACHED";

	/**
	 * Currently open {@link UsbControlBlock} instances.
	 */
	...
	/**
	 * Handler that dispatches callbacks on a worker thread.
	 */
	private final Handler mAsyncHandler;
	private volatile boolean destroyed;
	/**
	 * Callback listener for USB device attach/detach and related events.
	 */
	public interface OnDeviceConnectListener {
		...
		/**
		 * called when device detaches (after onDisconnect)
		 * @param device
		 */
		public void onDettach(UsbDevice device);
		/**
		 * called after device opens
		 * @param device
		 * @param ctrlBlock
		 * @param createNew
		 */
	...
	public void destroy() {
		...
		if (!destroyed) {
			destroyed = true;
			// Close every USB device we are monitoring
	...
	public synchronized void unregister() throws IllegalStateException {
		// Remove the periodic connection-check Runnable
		mDeviceCounts = 0;
		if (!destroyed) {
...
//			if (DEBUG) Log.i(TAG, "unregister:");
	 ...
	 * @param device
	 * @return true if the given {@link UsbDevice} has permission
	 * @throws IllegalStateException
	 */
	...
	/**
	 * Updates the internally cached permission state for {@code device}.
	 * @param device
	 * @param hasPermission
...
//		if (DEBUG) Log.v(TAG, "requestPermission:device=" + device);
		...
		if (isRegistered()) {
			if (device != null) {
				if (mUsbManager.hasPermission(device)) {
					...
					try {
						// No permission yet — request it from the user
						mUsbManager.requestPermission(device, mPermissionIntent);
					} catch (final Exception e) {
						// Some Android 5.1.x Galaxy devices throw a cryptic android.permission.sec.MDM_APP_MGMT exception here
						Log.w(TAG, e);
	...
	/**
	 * Open the given {@link UsbDevice}.
	 * @param device
	 * @return
	 * @throws SecurityException if permission has not been granted
	 */
	public UsbControlBlock openDevice(final UsbDevice device) throws SecurityException {
		if (hasPermission(device)) {
			...
			if (result == null) {
				result = new UsbControlBlock(USBMonitor.this, device);    // Constructor opens the device connection
	...
	/**
	 * Close and remove any cached {@link UsbControlBlock} for this device so the next
	 * {@link #openDevice(UsbDevice)} creates a new {@link android.hardware.usb.UsbDeviceConnection}.
	 * <p>
	 * Call after a failed or incomplete UVC session so libusb can recover from a stuck interface
	 * (-errno 22 / {@code open failed:result=-99} on the next open).
	 */
	public void releaseCachedDevice(final UsbDevice device) {
		if (destroyed || device == null) {
			return;
		}
		final UsbControlBlock ctrlBlock = mCtrlBlocks.remove(device);
		if (ctrlBlock != null) {
			try {
				ctrlBlock.close();
			} catch (final Exception e) {
				Log.w(TAG, "releaseCachedDevice: close failed", e);
			}
		}
	}
	...
	/**
	 * Builds a stable device key string for persisting settings per USB device.
	 * Derived from vendor ID, product ID, device class, device subclass, and device protocol.
	 * Identical hardware models therefore share the same key — plan accordingly.
	 * @param device null returns an empty string
	 * @return
	 */
	...
	/**
	 * Builds a stable device key string for persisting settings per USB device.
	 * With {@code useNewAPI=false}, identical models collide on the same key — plan accordingly.
	 * @param device
	 * @param useNewAPI
	 * @return
	 */
	...
	/**
	 * Builds a stable device key string for persisting settings per USB device (suitable as a {@link java.util.HashMap} key).
	 * Meaningful only while the {@link UsbDevice} is open when extra fields are required.
	 * Base fields: vendor ID, product ID, device class, device subclass, and device protocol.
	 * If {@code serial} is non-null and non-empty, it is appended to the key.
	 * When {@code useNewAPI=true} and the API level allows, manufacturer name, device version, and configuration count are included.
	 * @param device null returns an empty string
	 * @param serial serial from {@link UsbDeviceConnection#getSerial}; pass null — if {@code useNewAPI=true} and API &gt;= 21, it may be fetched internally
...
//		if (DEBUG) Log.v(TAG, "getDeviceKeyName:" + sb.toString());
	...
	/**
	 * Returns the device key as an integer ({@link String#hashCode()} of {@link #getDeviceKeyName(UsbDevice)}).
	 * Derived from vendor ID, product ID, device class, device subclass, and device protocol.
	 * Identical hardware models therefore share the same key — plan accordingly.
	 * @param device null returns 0
	 * @return
	 */
	...
	/**
	 * Returns the device key as an integer ({@link String#hashCode()} of the key name from {@link #getDeviceKeyName(UsbDevice, boolean)}).
	 * With {@code useNewAPI=false}, identical models collide on the same key — plan accordingly.
	 * @param device
	 * @param useNewAPI
	 * @return
	...
	/**
	 * Returns the device key as an integer ({@link String#hashCode()} of the key name from {@link #getDeviceKeyName(UsbDevice, String, boolean)}).
	 * With {@code serial} null and {@code useNewAPI=false}, identical models collide on the same key — plan accordingly.
	 * @param device null returns 0
	 * @param serial serial from {@link UsbDeviceConnection#getSerial}; pass null — if {@code useNewAPI=true} and API &gt;= 21, it may be fetched internally
	 * @param useNewAPI when true, also uses APIs available only on API 21+ or 23+ (some devices return null; behavior varies by hardware)
	 * @return
	...
	/**
	 * Reads a UTF-16LE string from the USB string descriptor with index {@code id}; returns null if unavailable.
	 * @param connection
	 * @param id
	 * @param languageCount
	 * @param languages
		...
		for (int i = 1; i <= languageCount; i++) {
			...
			if ((ret > 2) && (work[0] == ret) && (work[1] == USB_DT_STRING)) {
				...
				try {
					...
					if (!"Љ".equals(result)) {	// Some devices return garbage for this descriptor
						break;
					} else {
						result = null;
					}
	...
	/**
	 * Fetches manufacturer, product, version, and serial for {@code device}.
	 * @param device
	 * @return
	 */
	...
	/**
	 * Fetches manufacturer, product, version, and serial for {@code device}.
	 * Helper for {@link #updateDeviceInfo(UsbManager, UsbDevice, UsbDeviceInfo)}.
	 * @param context
	 * @param device
	 * @return
	 */
	...
	/**
	 * Fills manufacturer, product, version, and serial into {@code _info} (or a new instance when null).
	 * @param manager
	 * @param device
	 * @param _info
	 * @return
			...
			if ((manager != null) && manager.hasPermission(device)) {
				...
				try {
					...
					if (connection != null) {
						...
						final int result = connection.controlTransfer(
							USB_REQ_STANDARD_DEVICE_GET, // USB_DIR_IN | USB_TYPE_STANDARD | USB_RECIP_DEVICE
		    				USB_REQ_GET_DESCRIPTOR,
						...
						if (languageCount > 0 && desc != null && desc.length >= 17) {
	...
	public static final class UsbControlBlock implements Cloneable {
		...
		private UsbControlBlock(final USBMonitor monitor, final UsbDevice device) {
			...
			if (mConnection == null) {
				throw new IllegalStateException("failed to open UsbDeviceConnection");
			}
...
//			if (DEBUG) {
				if (mConnection != null) {
					...
					if (DEBUG) Log.i(TAG, String.format(Locale.US, "name=%s,desc=%d,busnum=%d,devnum=%d,rawDesc=", name, desc, busnum, devnum) + rawDesc);
				} else {
...
//			}
		...
		private UsbControlBlock(final UsbControlBlock src) throws IllegalStateException {
			...
			mBusNum = src.mBusNum;
			mDevNum = src.mDevNum;
			// FIXME: copy constructor should register with USBMonitor.mCtrlBlocks (HashMap would replace entries; consider List or map of lists)
		}
