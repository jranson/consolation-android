...
import org.centennialoss.consolation.uvc.USBMonitor.UsbControlBlock;
...
public class UVCCamera {
	...
	public static final int PROCESSING_STATS_COUNT = 30;
	...
	/**
	 * Linux usbfs reset (same ioctl as {@code libusb_reset_device} on Android).
	 * Call only when the device is not in use by an open {@code UVCCamera} (all interfaces
	 * released / connection not shared with active streaming).
	 *
	 * @return 0 on success, or a negative errno-style value (e.g. -EBUSY) on failure.
	 */
	public static int ioctlUsbReset(final UsbDeviceConnection connection) {
		if (connection == null) {
			return -1;
		}
		...
		return nativeIoctlUsbReset(fd);
	}

	private static native int nativeIoctlUsbReset(int fd);

	private UsbControlBlock mCtrlBlock;
	/** Owned when using [setPreviewTexture]; released in [close]. */
	private Surface mPreviewSurface;
    protected long mControlSupports;			// Bitmask of supported camera control unit capabilities
    protected long mProcSupports;				// Bitmask of supported processing unit capabilities
    ...
    public synchronized void open(final UsbControlBlock ctrlBlock) {
		...
		if (result != 0) {
			// nativeConnect failed: release cloned UsbControlBlock immediately to avoid
			// leaking UsbDeviceConnection/file descriptor on the Java side.
			if (mCtrlBlock != null) {
				try {
					mCtrlBlock.close();
				} catch (final Exception closeErr) {
					Log.w(TAG, "open: failed to close control block after connect failure", closeErr);
				}
				mCtrlBlock = null;
			}
...
//    		mNativePtr = 0;	// Do not clear here: nativeRelease ends up calling nativeDestroy
    	}
    	if (mCtrlBlock != null) {
			mCtrlBlock.close();
   			mCtrlBlock = null;
	...
	/**
	 * Full libusb/UVC descriptor tree as JSON (same content as native diagnostics).
	 * For Logcat debugging when formats do not appear in [getSupportedSizeList].
	 */
	public synchronized String getUvcDescriptorsJson() {
		if (mNativePtr == 0) {
			return "";
		}
		final String s = nativeGetDescriptions(mNativePtr);
		return s != null ? s : "";
	}
	...
	public List<Size> getSupportedSizeList() {
		/* -1 = include every format (MJPEG, uncompressed, frame-based, stream-based) so libuvc-reported modes all appear. */
		return getSupportedSize(-1, getSupportedSize());
	}
	...
	public static List<Size> getSupportedSize(final int type, final String supportedSize) {
		...
		if (!TextUtils.isEmpty(supportedSize)) {
			try {
				...
				if (DEBUG) Log.w(TAG, "Failed to parse supported size JSON", e);
			}
		}
		return result;
	}
	...
	private static final void addSize(final JSONObject format, final int formatType, final int frameType, final List<Size> size_list) throws JSONException {
		...
		final JSONArray intervalsOuter = format.optJSONArray("intervals");
		final int previewFrameFormat = format.optInt("frameFormat", frameType);
		...
		for (int j = 0; j < size_nums; j++) {
			...
			try {
				...
				int[] intervalInts = null;
				if (intervalsOuter != null && j < intervalsOuter.length()) {
					final JSONArray intervalArr = intervalsOuter.optJSONArray(j);
					if (intervalArr != null && intervalArr.length() > 0) {
						intervalInts = new int[intervalArr.length()];
						for (int k = 0; k < intervalArr.length(); k++) {
							intervalInts[k] = intervalArr.getInt(k);
						}
					}
				}
				if (intervalInts != null && intervalInts.length > 0) {
					size_list.add(new Size(formatType, previewFrameFormat, j, w, h, intervalInts));
				} else {
					size_list.add(new Size(formatType, previewFrameFormat, j, w, h));
				}
			} catch (final Exception e) {
				break;
			}
    ...
    public synchronized void setPreviewTexture(final SurfaceTexture texture) {	// API >= 11
    	...
    	nativeSetPreviewDisplay(mNativePtr, mPreviewSurface);
    }
	...
	public synchronized long[] getAndResetProcessingStats() {
		...
		return new long[PROCESSING_STATS_COUNT];
	}
    ...
    // it is better to wait several hundreads milliseconds.
...
//================================================================================
	...
	public synchronized boolean getAutoWhiteBalance() {
    	boolean result = true;
    	if (mNativePtr != 0) {
    		result = nativeGetAutoWhiteBalance(mNativePtr) > 0;
    	}
    	return result;
    }
	...
	public synchronized void updateCameraParams() {
    	if (mNativePtr != 0) {
    		if ((mControlSupports == 0) || (mProcSupports == 0)) {
        		// Load supported capability flags if still zero
    	    	...
    	    	// Load per-control min/max/def from the device
    	    	if ((mControlSupports != 0) && (mProcSupports != 0)) {
	    	    	nativeUpdateBrightnessLimit(mNativePtr);
	    	    	nativeUpdateContrastLimit(mNativePtr);
	    	    	nativeUpdateSharpnessLimit(mNativePtr);
...
//**********************************************************************
	...
	private final native int nativeUpdateAutoWhiteBalanceLimit(final long id_camera);
    private static final native int nativeSetAutoWhiteBalance(final long id_camera, final boolean autoWhiteBalance);
    private static final native int nativeGetAutoWhiteBalance(final long id_camera);

    private final native int nativeUpdateAutoWhiteBalanceCompoLimit(final long id_camera);
    private static final native int nativeSetAutoWhiteBalanceCompo(final long id_camera, final boolean autoWhiteBalanceCompo);
    private static final native int nativeGetAutoWhiteBalanceCompo(final long id_camera);

	private final native int nativeUpdateWhiteBalanceLimit(final long id_camera);
    private static final native int nativeSetWhiteBalance(final long id_camera, final int whiteBalance);
    private static final native int nativeGetWhiteBalance(final long id_camera);

	private final native int nativeUpdateWhiteBalanceCompoLimit(final long id_camera);
    private static final native int nativeSetWhiteBalanceCompo(final long id_camera, final int whiteBalance_compo);
    private static final native int nativeGetWhiteBalanceCompo(final long id_camera);
